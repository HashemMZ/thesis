%Viterbi with soft decision LVB
...
%             [tj,s_j]
            if tj>lenC %| (tj==lenC & s_j~=ST)%ignore final transitions that will go further than the whole frame
                continue
            end
...
for state=1:sNo%show costs    
    ...
    highcost(state)=s_info(state,t).cost(1);%sum([s_info(state,t).cost]/length(s_info(state,t).cost));
end
[highcost,hCostind]=sort(highcost,'descend');
% stem(highcost)