function [code,state] = sqarithencoflush1(symb,counts,state,N,E3_max,midFS,EOB)
%
...
counts1=bordercmpt(state,counts(1));
counts2=bordercmpt(state,counts(1)+counts(2));
if length(counts)==2 
    counts=counts1;
elseif length(counts)==3 
    counts=[counts1(1) state(2)-state(1)+1-counts2(end)-counts1(1) counts2(end)];    
end
