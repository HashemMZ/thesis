...
% load data
% format
counts=[20 6 15];ENo=12;%counts=[2/11 9/11];%Pep=.5;counts=[counts-counts*Pep Pep]*2^N-1;
...
while 1
    ...
    if isempty(reply) || reply=='y'
        eval( ['F:\2\load loader1 ']);
        eval( ['F:\2\load siml', int2str(siml)]);
        start=siml+1
        stop=input('stop? ');
        break
    elseif reply == 'n';
...
while siml<=stop
    ...
    %     pause(0.5)
    if rem(siml,every)==0
        eval( ['save F:\2\siml', int2str(siml),' siml shP shN shZ fi every']);
        eval( ['save F:\2\loader1 ','siml']);
        figure(1);
